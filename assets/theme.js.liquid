...
timber.drawersInit = function() {
  ...
  timber.FilterDrawer = new timber.Drawers('sgCollectionFilter', 'left');
...
theme.cart = ShopifyAPI.getCart((cart) => {
  ...
  document.dispatchEvent(new CustomEvent("sg:cartLoaded"));
});
...
theme.ajaxCart = (function(module, $) {
  ...
  loadCart = function () {
    ...
    if (theme.cart) {
      ...
      document.addEventListener("sg:cartLoaded", () => {
        window.initNotificationToast = false;
        cartUpdateCallback(theme.cart);
      });
    }
  ...
  updateCountPrice = function(cart) {
    ...
    if (theme.settings.sgCartSavings) {
      return;
    }
...
theme.discountCode = {
  ...
  triggerStorageDiscountUpdated: function() {
    document.dispatchEvent(new CustomEvent("sg:discountUpdated"));
  }
};
...
//_seaggs
theme.seaggs = function() {
  let addonsLoaded = false;

  document.addEventListener("sg:loaded", function(event) {
    if (addonsLoaded) {
      return;
    }
  ...
  document.body.classList.add("sg-collection_addtocart-destroy");
  document.body.classList.add("sg-collection_filters-destroy");
  document.body.classList.add("sg-color_swatches-destroy");
  document.body.classList.add("sg-cookie_box-destroy");
  document.body.classList.add("sg-delivery_date-destroy");
  document.body.classList.add("sg-delivery_time-destroy");
  document.body.classList.add("sg-faq_page-destroy");
  document.body.classList.add("sg-instagram_feed-destroy");
  document.body.classList.add("sg-mega_menu-destroy");
  document.body.classList.add("sg-newsletter_popup-destroy");
  document.body.classList.add("sg-product_image_crop-destroy");
  document.body.classList.add("sg-quantity_breaks-destroy");
  document.body.classList.add("sg-quick_compare-destroy");
  document.body.classList.add("sg-quick_view-destroy");
  document.body.classList.add("sg-sticky_addtocart-destroy");
  document.body.classList.add("sg-trust_badge-destroy");
  document.body.classList.add("sg-upsell_popup-destroy");
  document.body.classList.add("sg-wish_list-destroy");
};
...
theme.showTheMostValuableDiscountInCart = function() {
  ...
  function init(cart) {
    ...
    if (!mostValuableDiscount) {
      ...
      document.dispatchEvent(new CustomEvent("sg:cartDiscountSaved"));
    ...
    var currencySelector = $("#currency-list-header[name='sg-custom-currencies']"),
...
theme.syncedProductVariant = function (select, type) {
  ...
  const swatchesIsPresent = !!document.querySelector(".sg-color_swatches");
...
theme.requiredToCheckoutAddons = (function() {
  ...
  function init() {
    // Agree to terms
    if (addonsList.sg_agree_to_terms) {
      requiredToCheckoutAddons.push("agree-to-terms");
    }

    // Delivery date
    if (addonsList.sg_delivery_date && theme.settings.sgDeliveryDateRequired) {
          ...
          if (variant.available) {
            ...
            if (variant.inventory_management === "shopify" && variantInventoryPolicy !== "continue") {
              ...
              if (variant.inventory_management === "shopify" && variantInventoryPolicy === "continue" && variantInventoryQuantity <= 0) {
                $(".sg-quantity_breaks", this.$container).find(".qb-quantity").show();
              }
            }
          } else {
            productUnavailableToBuy.bind(this)();
...
theme.Collection = (function() {
  function Collection(container) {
    ...
    theme.ProductGridSlider($("#shopify-section-sg-collection-filters"));
